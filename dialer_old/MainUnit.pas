...
begin
  ...
  begin
    randomize;
    CheckUpdate.Interval := 300000 + Random(600000);
    if MessageBoxW(Handle, PWideChar(UpdateInfo), PWideChar(
      UTF8Decode('Dianet Dialer. Доступно обновление. Скачать?')), MB_ICONINFORMATION + MB_YESNO) = idYes then
          ShellExecute(ConfigForm.Handle, 'open', PChar(UpdateLink), nil, nil, SW_SHOWMAXIMIZED);
  end;
end;
